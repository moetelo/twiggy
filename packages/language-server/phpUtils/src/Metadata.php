...
class Metadata {
	 ...
	 * @param array<string,string[]> &$loaderPaths Array where key is namespace and value is an array of FS paths.
	 * @param \Twig\Loader\LoaderInterface $loader Loader.
	 */
	public static function mapNamespaces(array &$loaderPaths, \Twig\Loader\LoaderInterface $loader): void {
		if ($loader instanceof \Twig\Loader\ChainLoader) {
			foreach ($loader->getLoaders() as $subLoader) {
				static::mapNamespaces($loaderPaths, $subLoader);
			}
	...
	public static function getTwigMetadata(\Twig\Environment $twig, bool $map_namespaces = true): array {
		$loaderPathsArray = [];
		if ($map_namespaces) {
			static::mapNamespaces($loaderPathsArray, $twig->getLoader());
		}
		...
		return [
			'functions' => array_reduce(
				$twig->getFunctions(),
				fn ($acc, $item) => $acc + [$item->getName() => static::getArguments('functions', $item)],
				[],
			),
			'filters' => array_reduce(
				$twig->getFilters(),
				fn ($acc, $item) => $acc + [$item->getName() => static::getArguments('filters', $item)],
				[],
			),
			'tests' => array_values(
				array_map(
	...
	public static function getArguments(string $type, \Twig\TwigFunction|\Twig\TwigFilter $entity): mixed {
